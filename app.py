...
def download_gdrive_file(url):
    try:
        ...
        # Use a session for retries (good for robustness)
...
def get_recipes_api():
    ...
    try:
        # 1. Get ingredients from the webpage
        data = request.json
        ...
        # 3. Create the user's ingredient vector (the "one-hot" vector)
        ...
        if not found_ingredients:
            return jsonify([]) # Return empty list if no ingredients matched
        ...
        for index in top_matches_indices:
            if scores[index] > 0.05: # Only show decent matches
...
# This is for Render to run the app (it replaces app.run())
